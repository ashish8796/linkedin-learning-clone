...
export const addChapter =async (req:Request, res:Response) :Promise<void>=>{
    try {
        ...
        let body=req.body as Pick<IChapter, "title"|"description" | "createdAt" | "authorId" | "courseId" |"content"|"videosId">
        console.log(body)
...
export const updateChapter=async (req: Request, res: Response):Promise<void>=>{
    try{
        ...
        console.log(body,id)
        const updatedChapter:IChapter|null = await chapter.findByIdAndUpdate({_id:id},body)
...
export const getChapterId = async(req: Request, res: Response): Promise<void>=> {
    try {
        const {params:{id}} =req;
...
export const deleteChapter = async (req: Request, res: Response):Promise<void>=> {
    try{
        const delete_chapter:IChapter|null = await chapter.findByIdAndRemove(req.params.id)
        const allChapters:IChapter[]= await chapter.find()
