...
export const addUser =async (req: Request, res: Response) : Promise<void>=> {
    try {
        let body= req.body as Pick<IUser, "firstName"|"lastName"|"qualification"|"savedCourseId"|"interests"|"startOfProgram"|"emailId"|"password"|"Image"|"flag">;
...
export const updateUser=async (req: Request, res: Response):Promise<void>=>{
    try{
        ...
        console.log(body,id)
        const updatedStudent:IUser|null = await user.findByIdAndUpdate({_id:id},body)
...
export const deleteUser = async (req: Request, res: Response):Promise<void>=> {
    try{
        const delete_student:IUser|null = await user.findByIdAndRemove(req.params.id)
        const allStudents:IUser[]= await user.find()
